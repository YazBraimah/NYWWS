...
lineages_metadata_gisaid <- snakemake@input[["lineage_map_gisaid"]]
...
output_file_gisaid <- snakemake@output[["rds_data_gisaid"]]
...
for(i in unique(weeks)){
  ...
  # LIST THE VOC'S IN THE SEWERSHED
  var_sewer.voc_2 <- gs_data_recent %>%
    group_by(sw_id) %>%
    filter(!is.na(lineage)) %>%
    filter(monitored == "Monitored") %>%
  ...
  # paste together the variants if they have multiple
  var_sewer.voc_2 <- aggregate(lineage ~ sw_id, unique(var_sewer.voc_2), paste, collapse = ", ")
  colnames(var_sewer.voc_2) <- c("sw_id", "vocs_detected")
  var_sewer.voc_2$year_week <- as.Date(i, origin = "1970-01-01")
  
  # store in list
  voc_data_list[[i]] <- var_sewer.voc_2
  
}

# transform into df
var_sewer.voc_2 <- do.call(rbind, voc_data_list)
colnames(var_sewer.voc_2) <- c("sw_id", "vocs_detected_lin", "year_week")
...
# merge voc's detected to sewer list
var.data_summary <- left_join(var.data_summary, var_sewer.voc, by = c("sw_id", "year_week")) 

# merge voc lineages detected to sewer list
var.data_summary <- left_join(var.data_summary, var_sewer.voc_2, by = c("sw_id", "year_week"))
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# ---------------------------------------------------------- WASTEWATER TO GISAID MAPPING FILE ---------------------------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# WASTEWATER TO GISAID MAPPING 
# PURPOSE: TO MATCH CLINCIAL GROUPINGS FOR DOH CLINCIAL TO WASTEWATER COMPARISONS

# This script creates a second var.data_summary object called var.data_summary_gisaid for use in creating the second aggregation figures
...
# lineage map
lineage.map_colors = import.data(lineages_metadata) |> 
  rename(hex = hex_code,
         variant = lineage,
         lineage = callout_group) %>%
  dplyr::select(hex, lineage) %>%
  distinct()

# GISAID LINEAGE MAP
lineage.map = import.data(lineages_metadata_gisaid) |> 
  rename(
         variant = lineage,
         lineage = callout_group)

# add hex colors from NWSS
lineage.map <- left_join(lineage.map, lineage.map_colors, by = c("lineage"))
...
# merge voc lineage detections to sewer list
var.data_summary <- left_join(var.data_summary, var_sewer.voc_2, by = c("sw_id", "year_week")) 


# # save to RDS format
saveRDS(var.data_summary, file = output_file_gisaid)
